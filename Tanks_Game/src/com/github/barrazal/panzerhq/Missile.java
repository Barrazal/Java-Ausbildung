...
public class Missile extends GameObject {
    ...
    public void paintMe(java.awt.Graphics g){
        ...
        RoundRectangle2D missleShape = new RoundRectangle2D.Double (getObjectPosition().getX(),
                                                                    getObjectPosition().getY(),
                                                                    getWidth(),getHeight(), 3,3);

        transform.rotate(getMovingAngle(),missleShape.getCenterX(),missleShape.getCenterY());
        Shape transformedMissleShape = transform.createTransformedShape(missleShape);

        g2d.fill(transformedMissleShape);
